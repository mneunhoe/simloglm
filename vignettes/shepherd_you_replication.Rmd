...
results_ova <-
  simloglm(
    m4,
    nsim_est = 20000,
    scenario = list(futurelob = c(
      mean(m4$model$futurelob), mean(m4$model$futurelob) + sdxresid
    )),
...
fd_aca <-
  exp(mean(log(results_aca$geometric_mean[, 2, 1]))) - exp(mean(log(results_aca$geometric_mean[, 1, 1])))


# Ratio instead of fd
# The minus 1 offset matters here (doesn't for the fd above)
percent_increase_aca_dist <-
  ((results_aca$geometric_mean[, 2, 1] - 1) / (results_aca$geometric_mean[, 1, 1] - 1)) -
  1
percent_increase_aca <-
  ((exp(mean(
    log(results_aca$geometric_mean[, 2, 1])
  )) - 1) / (exp(mean(
    log(results_aca$geometric_mean[, 1, 1])
  )) - 1)) - 1
...
# Results with observed value approach


```{r Observed Value Plot, fig.width=5* (1 + sqrt(5)) / 2, fig.height=5, fig.path="shepherd_you_figures/", dev = c("png","pdf"), warning=FALSE}
# Summarize results

# First Difference as difference between expected values on the original scale
fd_dist <-
  results_ova$geometric_mean[, 2] - results_ova$geometric_mean[, 1]
fd <-
  exp(mean(log(results_ova$geometric_mean[, 2]))) - exp(mean(log(results_ova$geometric_mean[, 1])))


# Ratio instead of fd
# The minus 1 offset matters here (doesn't for the fd above)
percent_increase_dist <-
  ((results_ova$geometric_mean[, 2] - 1) / (results_ova$geometric_mean[, 1] - 1)) -
  1
percent_increase <-
  ((exp(mean(
    log(results_ova$geometric_mean[, 2])
  )) - 1) / (exp(mean(
    log(results_ova$geometric_mean[, 1])
  )) - 1)) - 1
...
plot(
  ...
  xaxt = "n",
  xlab = "",
  xlim = pretty(c(min(fd_dist), max(fd_dist)))[c(1, length(pretty(c(
    min(fd_dist), max(fd_dist)
  ))))]
)
title("",
      xlab = "Difference in Legislative Effectiveness Scores (LES)",
...
axis(
  1,
  at = c(min(fd_dist), max(fd_dist)),
  labels = round(c(min(fd_dist), max(fd_dist)), 2),
  lwd = 0,
  lwd.ticks = 1,
  col = "grey",
  col.axis = "grey"
...
axis(
  1,
  at = pretty(c(min(fd_dist), max(fd_dist)))[c(2:(length(pretty(c(
    min(fd_dist), max(fd_dist)
  ))) - 1))],
  lwd = 0.5,
  lwd.ticks = 1,
...
segments(
  x0 = quantile(fd_dist, 0.1),
  y0 = 1,
  x1 = quantile(fd_dist, 0.9),
  col = adjustcolor(viridis::viridis(2)[1], 1),
  lwd = 8,
  lend = 1
)
segments(
  x0 = quantile(fd_dist, 0.025),
  y0 = 1,
  x1 = quantile(fd_dist, 0.975),
  col = adjustcolor(viridis::viridis(2)[1], 1),
  lwd = 4,
  lend = 1
)
segments(
  x0 = quantile(fd_dist, 0.005),
  y0 = 1,
  x1 = quantile(fd_dist, 0.995),
  col = adjustcolor(viridis::viridis(2)[1], 1),
  lwd = 2,
  lend = 1
)
points(mean(fd_dist),
       ...
       pch = 1,
       col = viridis::viridis(2)[2])
points(fd, 1, pch = 124, col = viridis::viridis(2)[2])
legend(
  "top",
  legend = c("GM", "mean", "80% CI", "95% CI", "99% CI"),
  pch = c(124, 1, NA, NA, NA),
...
points(mean(fd), 1, pch = 1, col = viridis::viridis(2)[2])
points(percent_increase, 1, pch = 124, col = viridis::viridis(2)[2])
...
quantile(percent_increase_aca_dist, c(0.025, 0.975))
# One sided hypothesis test

mean(percent_increase_aca_dist > 0)
...
quantile(percent_increase_dist, c(0.025, 0.975))
```
