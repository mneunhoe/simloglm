...
vignette: >
  %\VignetteIndexEntry{Replication of Shepherd/You 2020}
...
# The dependent variable here is log(les + 1), so retransforming to the scale of
# les means subtracting the offset of 1 from every quantity of interest. Shift
# only the four result matrices and leave the rest of the simloglm object (and
# its class) alone.

shift_qoi <- function(sim_obj, offset) {
  for (nm in c("median", "mean", "median_point_estimate", "mean_point_estimate")) {
    sim_obj[[nm]] <- sim_obj[[nm]] + offset
  }
  sim_obj
}
```
...
# The scenario is passed as a model matrix rather than as a list because the
# congress and icpsr fixed effects are set to their observed proportions, which
# a scenario on the original scale cannot express. Columns of X are used exactly
# as supplied, so they must already be transformed -- which they are, having
# come straight out of model.matrix().

# lnles was logged before the model was fitted, so the formula cannot tell
# simloglm() that this is a log scale; logged_dv = TRUE (the default) says so.
...
# Summarize results
# First undo the offset of the log(les + 1) transformation
results_aca_retransformed <- shift_qoi(results_aca, -1)
...
# One sided test
mean(results_aca_retransformed$median[, 2] / results_aca_retransformed$median[, 1] -1  > 0)
```
