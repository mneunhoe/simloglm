...
vignette: >
  %\VignetteIndexEntry{Replication of Hollibaugh/Rothenberg 2018}
...
# The dependent variable was logged before the model was fitted, so the formula
# reads `logdist ~ ...` and simloglm() cannot tell from it that the model is on
# a log scale. logged_dv = TRUE (the default) is what we want here; we pass it
# explicitly so the intent is on the page.

# First we calculate the QoI with the original scenario of the authors.
# Only the variables that depart from the average case have to be named:
# everything else is held at its mean automatically.
...
# Second we choose an arbitrary extreme scenario. Here every value is chosen
# deliberately, so all of them are spelled out.
